...
import dev.kirill.kirevents.events.*;
...
public class EventManager {
    ...
    public EventManager(KirEvents plugin) {
        ...
        this.running = false; // Явно устанавливаем false
    ...
    public void startEvents() {
        if (running) {
            plugin.getLogger().warning("Events are already running!");
            return;
        }
        running = true;
        ...
        plugin.getLogger().info("Events system started!");
    }
    ...
    public void spawnEvent(EventType type, Location customLocation) {
        Location spawnLoc = customLocation != null ? customLocation : getRandomLocation();
    ...
    public EventStructure getStructureAt(Location loc) {
        for (EventStructure structure : activeStructures) {
            if (structure.containsBlock(loc) || structure.containsChest(loc)) {
                return structure;
            }
        }
        return null;
    ...
    public Set<EventStructure> getActiveStructures() {
        return new HashSet<>(activeStructures);
    }
    ...
    public void shutdown() {
        ...
        plugin.getLogger().info("Events system shutdown complete!");
    }
    ...
    public void stopEvents() {
        ...
        plugin.getLogger().info("Events stopped by command!");
    }
}